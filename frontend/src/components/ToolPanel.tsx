...
export function ToolPanel({
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-80 bg-slate-900/50 backdrop-blur-xl border-l border-slate-800/50">
        {/* 工具按鈕列 */}
        ...
        <div className="p-4 border-t border-slate-800/50 space-y-3">
          <button
            ...
            className="w-full h-12 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isProcessing ? (
              <>
                <Loader2 className="w-5 h-5 animate-spin" />
                <span>{processingStatus === 'uploading' ? `上傳中 ${progress}%` : '處理中...'}</span>
              </>
            ) : (
              <span>套用變更</span>
            )}
          ...
          {hasProcessedImage && (
            <button
              type="button"
              onClick={onDownload}
              className="w-full h-12 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white font-medium rounded-xl transition-colors"
            >
      ...
      <div className="lg:hidden fixed inset-x-0 bottom-0 z-50">
        ...
        <div
          className={`
            ...
            ${isExpanded && activeTool !== 'none' ? 'max-h-[70vh]' : 'max-h-[180px]'}
          `}
          ...
          <div className="px-4 pb-4 flex gap-3">
            ...
            {hasProcessedImage && (
              <button
                type="button"
                onClick={onDownload}
                className="h-12 px-6 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white font-medium rounded-xl transition-colors"
              >
                <Download className="w-5 h-5" />
              </button>
            )}
